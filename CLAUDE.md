...
| 7 | **Handoff = queue + operator reply + bot mute + release.** Handoff rows surface in the ops console and the bot goes silent on that conversation until released. **Revised in Phase 3:** the console is strictly read-only — *no mutation routes exist at all*, proven by an AST test over the whole console package — so the reply and release paths are `tools/handoff.py`, a CLI. The capability is unchanged; the console no longer stretches its own description. |
